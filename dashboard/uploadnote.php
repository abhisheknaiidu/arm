...
echo "<script>alert('file size is not proper');</script>";
        ...
        if(move_uploaded_file($_FILES['file']['tmp_name'], $folder.$notefile)) {
            ...
            if (mysqli_affected_rows($conn) > 0) {
                echo "<script> alert('file uploaded successfully.It will get published After ARM approves it');
...
<style>
	...
	<div class="form-group">
		...
		<input type="text" name="title" class="form-control" placeholder="Title of Document"  value = "<?php if(isset($_POST['upload'])) {
            echo $file_title; } ?>" required="">
	</div>

	<div class="form-group">
		<label for="post_tags">Description Of Document</label>
		<input type="text" name="description" class="form-control" placeholder="Description" value="<?php if(isset($_POST['upload'])) {
            echo $file_description;  } ?>" required="" ">
	</div>

	 <div class="form-group">
        <label for="post_image">Select File</label><font color="red"> (allowed file type: 'pdf','doc','ppt','txt','zip','png','jpg','jpeg'| allowed maximum size: 30 mb ) </font>
